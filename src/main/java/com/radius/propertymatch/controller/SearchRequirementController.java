		...
		try {
            ...
            while (parser.nextToken() != JsonToken.END_ARRAY) {
                ...
                if ("lat".equals(token)) {
                    ...
                    //To simulate if either min budget or max budget is not provided, setting it to null if random boolean gives true, else getting random integer within range.    
                    Integer minBudget = random.nextBoolean()?null:random.nextInt(priceRange) + minPrice;
                    
                    // If min budget gets set to null, then max budget cannot be null, hence added check to see min budget if null or not
                    Integer maxBudget = (minBudget!=null?(random.nextBoolean()?null:(random.nextInt(priceRange) + minPrice)): (random.nextInt(priceRange) + minPrice));
                    ...
                    Integer maxBedrooms = (minBedrooms!=null?(random.nextBoolean()?null:(random.nextInt(roomRange) + minRooms)): (random.nextInt(roomRange) + minRooms));
                    ...
                    Integer maxBathrooms = (minBathrooms!=null?(random.nextBoolean()?null:(random.nextInt(roomRange) + minRooms)): (random.nextInt(roomRange) + minRooms));
